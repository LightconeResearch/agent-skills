...
This tutorial gets you started with [ASTRA](https://astra-spec.org) — the
specification for recording an analysis's inputs, outputs, and methodological
decisions — and the [`astra` plugin](astra.md), which teaches your agent to
work with it. The full specification lives at
[astra-spec.org](https://astra-spec.org).
...
The [`astra` plugin page](astra.md) covers what the plugin ships, and the full
ASTRA documentation lives at [astra-spec.org](https://astra-spec.org) — the
