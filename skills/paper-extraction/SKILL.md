...
This skill runs the bundled `scripts/extract-paper-substrate.py` and the `astra` CLI (for `astra paper` operations). The Python script needs `uv` (or Python 3.11+); the `astra` CLI is **not bundled with the skill**. Confirm they resolve first — `command -v uv; command -v astra || echo "astra not found"` — and if `astra` is missing, install the toolchain (one package ships both `astra` and `lc`): `uv tool install lightcone-cli` (or `pipx install lightcone-cli`). Discover command syntax with `--help` rather than guessing.
