...
```bash
uvx --from lightcone-cli@x.y.z lc <command>
```
...
3. **Spec exists** → read `astra.yaml` and `CLAUDE.md`, then run
   `uvx --from lightcone-cli@x.y.z lc status` (for a large spec, the astra
...
Scaffold first if needed: `uvx --from lightcone-cli@x.y.z lc init [DIR]`
...
```bash
uvx --from lightcone-cli@x.y.z lc init [DIR] [--permissions yolo|recommended|minimal] [--scratch PATH]  # Scaffold a new project
uvx --from lightcone-cli@x.y.z lc run [OUTPUTS...] [--universe NAME] [--force] [--verbose] [--rerun-triggers TRIGGERS]  # Materialize outputs
uvx --from lightcone-cli@x.y.z lc build [--force] [--runtime docker]   # Build container images from specs
uvx --from lightcone-cli@x.y.z lc status [--universe NAME] [--json]    # Materialization status
uvx --from lightcone-cli@x.y.z lc verify [--universe NAME]             # Recompute hashes, walk the provenance chain
uvx --from lightcone-cli@x.y.z lc export wrroc [--output PATH] [--universe NAME] [--zip] [--metadata-only] [--author "NAME <EMAIL>"]  # Export RO-Crate bundle
```
...
After a fix: `uvx --from lightcone-cli@x.y.z lc run <output_id> --universe <name>`.
...
```bash
uvx --from lightcone-cli@x.y.z lc export wrroc                              # ./wrroc/ directory
uvx --from lightcone-cli@x.y.z lc export wrroc -o run.zip --zip             # zip bundle
uvx --from lightcone-cli@x.y.z lc export wrroc --metadata-only              # no data files
uvx --from lightcone-cli@x.y.z lc export wrroc -u baseline -u alt_method    # specific universes
uvx --from lightcone-cli@x.y.z lc export wrroc --author "Name <email@host>" # override git config
```
