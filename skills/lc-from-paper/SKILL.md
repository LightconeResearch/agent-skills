...
This skill orchestrates the `lc` and `astra` CLIs (and the `/paper-extraction`, `/lc-from-code`, `/ralph` sibling skills), which are **not bundled here**. Confirm the toolchain resolves first — `command -v lc && command -v astra || echo "lightcone-cli toolchain not found"` — and if missing, install it (one package ships both): `uv tool install lightcone-cli` (or `pipx install lightcone-cli`). A reproduction lives in an `lc init` project. Discover command syntax with `--help` rather than guessing. For the fullest reproduction toolkit (this skill plus its siblings and the `lc-extractor` subagent), install the `lightcone-reproduction` plugin; see the repository README.
