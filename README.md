...
`lightcone-core`), use the native plugin paths below.
...
`lightcone-core` depends on `astra`; installing it pulls in the `astra` reference too.
This is the **only** path that ships the SessionStart/PostToolUse hooks and the
`lc-extractor` subagent (used by `lc-new` for literature extraction).

### Codex plugin

```bash
codex plugin marketplace add LightconeResearch/agent-skills
codex /plugins        # browse + install astra / lightcone-core
```
...
|---|---|---|
| **`astra`** | `astra` | — |
| **`lightcone-core`** | `lc-cli`, `lc-new`, `lc-from-code`, `lc-feedback` (+ `astra`) | venv-activation & validate-on-save hooks; `lc-extractor` subagent |
