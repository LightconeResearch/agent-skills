...
description: >
  ...
  `astra.yaml` — see the astra reference skill) and not for the HTML
...
Check the CLIs resolve (silently):

- `lc --help` and `astra --help` — if missing: `uv tool install lightcone-cli`.
- `myst --version` — if missing, tell the user: `npm install -g mystmd` (or `uv tool install mystmd`; Node ≥ 18). Drafting can proceed without it, but the validation loop below cannot — say so rather than skipping validation silently.
...
- **Only touch report pages and `myst.yml`.** Spec changes go through the normal `astra.yaml` discipline (see the astra reference skill), not through this skill. Never edit anything under `results/`.
